...
#
#
# 6.网址后缀改为.zip后不断迭代文件内容,最后提示利用zipfile搜集文件的comment
# 日了狗，得到hockey返回 it's in the air. look at the letters.  答案是氧气
import re
import zipfile

zpath = '/Users/Peterkwok/GitHub/pythonchallenge/channel.zip'
zfile = zipfile.ZipFile(zpath, mode='r')
result = []
...
def getContent(num):
    ...
    m = re.match('.*?(\d+).*', data)
    result.append(zfile.getinfo(fname).comment)
    if (m):
        print m.group(1)
        return getContent(m.group(1))
    else:
        return num


final = getContent('90052')
print ''.join(result)
zfile.close()
...
pic = Image.open('/Users/Peterkwok/GitHub/pythonchallenge/oxygen.png')
# 裁剪图片
area = (0, 43, 608, 52)
cpic = pic.crop(area)
# 化为像素点信息 - > list
data = cpic.convert('L').getdata()
result = ''
...
pic = Image.open('/Users/Peterkwok/GitHub/pythonchallenge/good.jpg')
...
dpic = ImageDraw.Draw(pic)
dpic.line(first, fill='black')
dpic.line(second, fill='black')
pic.show()
pic.close()

# for x in range(0, len(first), 2):
#     pic.putpixel((first[x], first[x + 1]), (255, 0, 0))
# for x in range(0, len(second), 2):
#     pic.putpixel((second[x], second[x + 1]), (255, 0, 0))
# pic.show()
# pic.save('/Users/Peterkwok/GitHub/pythonchallenge/bull.jpg')
# pic.close()
